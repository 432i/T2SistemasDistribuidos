...
func generarPropuesta(cantPartes string) {
	...
	for i < partes {
		...
		if (se_pudo2 == true && se_pudo3 == true) {
			...
			escribirLogNN(cola_chunks_de_cliente[0].GetNombreLibro(), cola_chunks_de_cliente[0].GetTotalPartes(), cola_chunks_de_cliente[0].GetParte(), "10.6.40.149")
			mensajesDistribuido += 1
		}
...
func generarPropuestaCentralizado(cantPartes string, nombreLibro string){
	...
	i := 0
	for i < partes{
		propuesta := "DN1#"+cantPartes+"#"+nombreLibro //le enviamos la propuesta inicial donde asumimos q los demas datanodes estan activos
