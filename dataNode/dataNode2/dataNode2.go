...
Parametro:
    - c1, c3: Conexiones del DN con los otros dos
Descripcion:
	- Le envía al DN correspondiente el chunk (o a sí mismo no sortea), dependiendo del sorteo que se hace
Retorno:
	- No hay
*/
func propuestaEntreTres(c1 pb.NewChatCliDnClient, c3 pb.NewChatCliDnClient) {
	msg2 := pb.Message {
		Body: "m",
	}
	msg3 := pb.Message {
...
func generarPropuesta(cantPartes string) {
	...
	var connDN1, connDN3 *grpc.ClientConn
	...
	for i < partes {
		...
		if (se_pudo1 == true && se_pudo3 == true) {
			if estado == "buscada" {
				...
				fmt.Println(msj3)
				estado = "tomada"
				propuestaEntreTres(c1, c3)
			}
			if estado == "tomada" {
				propuestaEntreTres(c1, c3)
			}
		} else if (se_pudo1 == true && se_pudo3 == false) {
			if estado == "buscada" {
				msj1, _ := c1.EnviarPeticion(context.Background(), &mensajito)
				for msj1 != "ok" {
					msj1, _ = c1.EnviarPeticion(context.Background(), &mensajito)
		...
		} else if (se_pudo1 == false && se_pudo3 == true) {
			if estado == "buscada" {
				msj3, _ := c3.EnviarPeticion(context.Background(), &mensajito)
				for msj3 != "ok" {
					msj3, _ = c3.EnviarPeticion(context.Background(), &mensajito)
...
func generarPropuestaCentralizado(cantPartes string, nombreLibro string){
	...
	for i < partes{
		...
		propuesta := "DN2#DN1DN3#"+nombreLibro //le enviamos la propuesta inicial donde asumimos q los demas datanodes estan activos
...
Retorno:
	- No hay
*/
func serverDN2() { //Comunicacion con cliente
	//-----------------------------------------------------------------> Server1
