...
func generarPropuesta(cantPartes string) {
	...
	for i < partes {
		...
		if errFunc2 != nil {
			...
			fmt.Println(fun2.Body) //Exito
			fmt.Println("Conexion realizada correctamente con el Data Node de IP 10.6.40.149")
		}


		connDN3, _ := grpc.Dial("10.6.40.150:50001", grpc.WithInsecure())
		defer connDN3.Close()
		...
		if (se_pudo2 == true && se_pudo3 == true) {
			fmt.Println("Se establecio conexion con los DN con IPs 10.6.40.149 y 10.6.40.150")
			if estado == "buscada" {
			...
			fmt.Println("Se establecio conexion solamente con el DN de IP 10.6.40.150")
			if estado == "buscada" {
				msj3, _ := c3.EnviarPeticion(context.Background(), &mensajito)
				mensajesDistribuido += 1
				for msj3.Body != "ok" {
...
func main(){
	fmt.Println("El servidor Data Node 3")
